...
class SKOptSpec(MSpec(isArray=True, scalarMode=ScalarMetaMap.degenerateCategory)):
	class HyperparamsSpecsConverters:
		...
		uniform = lambda k, dist, tp: typeRemap[tp](dist.ppf(0), dist.ppf(1), name=k)
		_categorical = lambda k, categories: skopt.space.Categorical(categories, name=k)
