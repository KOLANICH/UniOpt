...
class EvoStraSpec(MSpec(isArray=True, scalarMode=ScalarMetaMap.noScalars, integerMode=IntegerMetaMap.noIntegers)):
	...
	class HyperparamsSpecsConverters:
		randint = lambda k, dist, tp: ("I", dist.a, dist.b)
		uniform = lambda k, dist, tp: (tp.__name__[0].upper(), dist.ppf(0), dist.ppf(1))
...
class EvoStra(GenericOptimizer):
	...
	def invokeScoring(self, fn: typing.Callable, pb: ProgressReporter, spaceSpec) -> np.ndarray:
		...
		es = EvolutionStrategy(initialPoint, evostraScore, population_size=self.popSize, sigma=self.std, learning_rate=self.learningRate, decay=1., num_threads=self.jobs)
