...
def loadPyShacGridSpec():
	from pyshac import DiscreteHyperParameter, UniformContinuousHyperParameter, NormalContinuousHyperParameter
	...
	class PySHACGridSpec(MSpec(isArray=True, scalarMode=ScalarMetaMap.degenerateCategory, integerMode=IntegerMetaMap.noIntegers)):
		...
		class HyperparamsSpecsConverters:
			...
			uniform = lambda k, dist, tp: UniformContinuousHyperParameter(k, dist.ppf(0), dist.ppf(1))
			norm = lambda k, dist, tp: NormalContinuousHyperParameter(k, dist.mean(), dist.std())
			_categorical = lambda k, categories: DiscreteHyperParameter(k, categories)
